    ...
    <>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-8">
          <Link
            href="/create"
            className="inline-flex items-center gap-2 px-6 py-3 bg-[#A4193D] text-white rounded-xl font-medium hover:bg-[#7D132E] transition-colors shadow-lg"
          >
        ...
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-[#A4193D] mb-4">{h1}</h1>
        ...
        <section className="mb-12">
          <h2 className="text-2xl font-semibold text-[#A4193D] mb-6">
            How It Works
          </h2>
          <div className="grid md:grid-cols-3 gap-4">
            {copy.home.howItWorks.steps.map((step, index) => (
              <div
                key={index}
                className="p-5 bg-white border border-[#FFDFB9] rounded-xl"
              >
                <div className="w-10 h-10 bg-[#A4193D] text-white rounded-lg flex items-center justify-center text-lg font-bold mb-3">
                  {step.number}
                </div>
                <h3 className="font-semibold text-[#A4193D] mb-2">
                  {step.title}
                </h3>
                <p className="text-[#A4193D] text-sm">{step.description}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mb-12">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {useCases.map((useCase, index) => (
              <div
                key={index}
                className="p-6 bg-[#FFF5EB] rounded-xl border border-[#FFDFB9]"
              >
                <h3 className="font-semibold text-[#A4193D] mb-2">
                  {useCase.title}
                </h3>
                <p className="text-[#A4193D] text-sm">{useCase.description}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        {prompts && prompts.length > 0 && (
          <section className="mb-12">
            ...
            <div className="space-y-4">
              {prompts.map((p, i) => (
                <div key={i} className="p-4 bg-white border border-[#FFDFB9] rounded-xl">
          ...
          <ul className="space-y-3">
            {commonMistakes.map((mistake, index) => (
              <li key={index} className="flex items-start gap-3">
                ...
                <span className="text-[#A4193D]">Avoid: {mistake}</span>
              </li>
            ))}
          </ul>
        </section>
        ...
        {/* Mid CTA */}
        <section className="text-center py-12 bg-gradient-to-br from-[#A4193D] to-[#7D132E] rounded-2xl mb-12">
